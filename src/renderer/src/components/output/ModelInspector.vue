...
function typeLabel(tv: TypedValue): string {
  return tv.type
}
...
<template>
  <div class="inspector" :style="{ fontFamily: '\'JetBrains Mono\', monospace', fontSize: '0.8rem' }">
    ...
    <div class="attrs">
      <div v-for="(val, key) in attrs(data as ModelOrObject)" :key="key" class="attr-row">
        ...
        <span class="attr-type">{{ typeLabel(val) }}</span>
      </div>
    </div>
  </div>
</template>
...
.badge-type { font-size: 0.7rem; background: rgba(189,147,249,0.12); color: var(--neon-purple); padding: 0 5px; border-radius: 3px; font-family: 'JetBrains Mono', monospace; }
